...
} from "./commands/models/constants/entityTypes";
import {
	CommandResponse,
	Employee,
...
const processStartEmployeeDetailError = (error: any, res: Response): void => {
	...
	res.status(error.status || 500).render(ViewNameLookup.EmployeeDetail, <
		EmployeeDetailPageResponse
	>{
...
const determineCanCreateEmployee = async (
	req: Request
): Promise<CanCreateEmployee> => {
	...
	return EmployeeExistsQuery.execute()
		.then(
			(
				employeeExistsCommandResponse: CommandResponse<boolean>
			): Promise<CommandResponse<ActiveUser>> => {
				employeeExists =
					employeeExistsCommandResponse.data != null &&
				...
				return ValidateActiveUser.execute(
		...
		.then(
			(
				activeUserCommandResponse: CommandResponse<ActiveUser>
			): CanCreateEmployee => {
				return <CanCreateEmployee>{
					employeeExists: employeeExists,
					isElevatedUser:
						activeUserCommandResponse.data != null &&
						EmployeeHelper.isElevatedUser(
							(<ActiveUser>activeUserCommandResponse.data)
								.classification
						),
				};
			}
...
export const start = async (req: Request, res: Response): Promise<void> => {
	...
	return determineCanCreateEmployee(req)
		.then((canCreateEmployee: CanCreateEmployee): void => {
			if (
				canCreateEmployee.employeeExists &&
...
export const startWithEmployee = async (
	req: Request,
	res: Response
): Promise<void> => {
	if (Helper.handleInvalidSession(req, res)) {
...
const saveEmployee = async (
	...
	return determineCanCreateEmployee(req)
		...
		.then(
			(saveEmployeeCommandResponse: CommandResponse<Employee>): void => {
