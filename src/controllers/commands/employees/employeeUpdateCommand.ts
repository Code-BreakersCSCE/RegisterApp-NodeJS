...
export async function updateEmployee(req: EmployeeSaveRequest) {
    ...
    if (validRequest.status == 200) {
        return DatabaseConnection.createTransaction().then(function (trans) {
            return employee.queryById(<string>req.id, trans).then(function (employee) {
                if (employee == null) {
                ...
                return employee.update(createUpdateObject(req),
                    <Sequelize.InstanceUpdateOptions>{ Transaction: trans })
                ...
                return Promise.reject(<CommandResponse<Employee>>{
                    ...
                    message: (err.message || Resources.getString(ResourceKey.EMPLOYEE_UNABLE_TO_SAVE))
                });
